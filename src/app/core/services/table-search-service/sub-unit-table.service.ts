...
import { SortDirection } from '../../helpers/table-sortable/sub-unit-advanced-sortable.directive';
...
function matches(tables: any, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term.toLowerCase()) 
        || tables.mainUnit.name.toLowerCase().includes(term.toLowerCase()); 
}
...
export class TableService {
    ...
    constructor(private pipe: DecimalPipe,private tableListService:TableListService) {
        ...
        this._search$.next();
        this.tableListService.getAll(SUBUNIT_PATH).subscribe(data => {
            this.content = data;
          });
    }
    /**
