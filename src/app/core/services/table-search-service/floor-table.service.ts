...
function matches(tables: any, term: string, pipe: PipeTransform) {
    ...
    || tables.other.toLowerCase().includes(term.toLowerCase())
    || tables.block.name.toLowerCase().includes(term.toLowerCase())
    || tables.build.name.toLowerCase().includes(term.toLowerCase())
...
export class TableService {
    ...
    constructor(private pipe: DecimalPipe,private tableListService:TableListService) {
        ...
        this._search$.next();
        this.tableListService.getAll(FLOOR_PATH).subscribe(data => {
            this.content = data;
          });
    }
    /**
