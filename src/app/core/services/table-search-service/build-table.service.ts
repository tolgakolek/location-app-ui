...
import { SortDirection } from '../../helpers/table-sortable/build-advanced-sortable.directive';
...
function matches(tables: any, term: string, pipe: PipeTransform) {
    ...
    || tables.gps.toLowerCase().includes(term.toLowerCase())
    || tables.address.toLowerCase().includes(term.toLowerCase())
    || tables.properties.toLowerCase().includes(term.toLowerCase())
    || tables.campus.name.toLowerCase().includes(term.toLowerCase())
    || tables.site.name.toLowerCase().includes(term.toLowerCase())
...
export class TableService {
    ...
    constructor(private pipe: DecimalPipe,private tableListService:TableListService) {
        ...
        this._search$.next();
        this.tableListService.getAll(BUILD_PATH).subscribe(data => {
            this.content = data;
          });
    }
    /**
