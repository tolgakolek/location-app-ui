...
import { SortDirection } from '../../helpers/table-sortable/floor-advanced-sortable.directive';
...
function matches(tables: any, term: string, pipe: PipeTransform) {
    ...
    || tables.map.toLowerCase().includes(term.toLowerCase())
    || tables.floor.name.toLowerCase().includes(term.toLowerCase())
    || tables.floor.block.name.toLowerCase().includes(term.toLowerCase())
    || tables.floor.block.build.name.toLowerCase().includes(term.toLowerCase())
    || tables.floor.block.build.campus.name.toLowerCase().includes(term.toLowerCase())
...
export class TableService {
    ...
    constructor(private pipe: DecimalPipe,private tableListService:TableListService) {
        ...
        this._search$.next();
        this.tableListService.getAll(ROOM_PATH).subscribe(data => {
            this.content = data;
          });
    }
    /**
