...
import { SortDirection } from '../../helpers/table-sortable/user-advanced-sortable.directive';
...
function matches(tables: any, term: string, pipe: PipeTransform) {
    ...
    const gender=(tables.gender == true) ? 'Erkek' : 'Kadın';
    return tables.name.toLowerCase().includes(term.toLowerCase()) 
    || tables.surname.toLowerCase().includes(term.toLowerCase())
    || tables.surname.toLowerCase().includes(term.toLowerCase())
...
export class TableService {
    ...
    constructor(private pipe: DecimalPipe,private tableListService:TableListService) {
        ...
        this._search$.next();
        this.tableListService.getAll(USER_PATH).subscribe(data => {
            this.content = data;
          });
    }
    /**
